...
from mira.ml.flash_attention import attend
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        # q, k, v are (B, S, H, D) here. The FlashAttention path (H100 FA3/FA2) consumes this layout
        # directly; it is used only on the full-sequence training/eval path. The incremental-decode
        # path (kv_cache set, q_len=1) stays on SDPA -- it is the correctness-critical inference path
        # and not throughput-bound -- so we disable flash whenever a KV-cache is in play.
        prefer_flash = kv_cache is None
        y = attend(
            q,
            k,
            v,
