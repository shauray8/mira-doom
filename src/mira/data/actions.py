...
class KeyVocab:
    ...
    def default_rl(cls, on_unknown: Literal["warn", "ignore", "error"] = "warn") -> "KeyVocab":
        return cls(DEFAULT_KEYS, on_unknown=on_unknown)
...
def lines_have_mouse(lines: Iterable[str | bytes]) -> bool:
    """True if any `.jsonl` line carries a `"mouse"` field. Cheap substring probe over raw bytes/str
    so the (Rocket League) keyboard-only path stays exactly as before -- no mouse tensor is built
    unless the data actually contains one (e.g. the Doom turn-delta stream)."""
    token_b, token_s = b'"mouse"', '"mouse"'
    for line in lines:
        if (token_b in line) if isinstance(line, bytes) else (token_s in line):
            return True
    return False
...
def tensorize_mouse(
    ...
    """Parse `.jsonl` `"mouse"` deltas into a float32 tensor of shape (n_steps, 2).

    Mirrors :func:`tensorize_actions` (same integer stride, same trailing-window contract) so the
    mouse stream stays frame-aligned with the keyboard and video. Deltas are SUMMED over each
    downsample window (they are displacements: summing preserves total movement, unlike the OR used
    for key holds). A missing/blank `"mouse"` field on a line contributes `[0, 0]`.
    """
    import torch
    ...
    for line in lines:
        ...
        window[0] += float(mouse[0])
        window[1] += float(mouse[1])
        count += 1
        if count == factor:
            steps.append(window)
            window = torch.zeros(2, dtype=torch.float32)
