...
This adapts :meth:`mira.data.DoomWebDataset.iter_clips` (the streaming read path) into
...
from .actions import DEFAULT_KEYS, KeyVocab
from .batch import VideoActionBatch
from .dataset import DoomWebDataset
...
class _VideoActionIterable(IterableDataset):
    ...
    def _my_shards(self, dataset: DoomWebDataset) -> list[str]:
    ...
    def _decode_sample(self, plan: dict[str, Any]) -> dict[str, Any]:
        ...
        # Analog/mouse channel: use the clip's parsed mouse stream when the dataset carries one
        # (Doom's continuous turn delta in channel 0), else zeros (keyboard-only data).
        if clip.mouse is not None:
            actions.mouse_movements = clip.mouse[p].unsqueeze(0).to(torch.float32)  # (1, T, 2)
        else:
    ...
    def _shard_view(self, dataset: DoomWebDataset, shard: str) -> DoomWebDataset:
    ...
    def _groups(self, dataset: DoomWebDataset, shard: str) -> Iterator[list[dict[str, Any]]]:
    ...
    def __iter__(self) -> Iterator[dict[str, Any]]:
        ...
        dataset = DoomWebDataset.from_local(self.index_path, vocab=self.vocab)
...
def create_loader(
    ...
    *,
    ...
    Args:
        ...
        valid_keys: Key vocabulary; defaults to ``DEFAULT_KEYS``. Ignored if ``action_config`` is
            given.
    ...
    if action_config is None:
        action_config = ActionConfig(
            valid_keys=list(valid_keys) if valid_keys is not None else list(DEFAULT_KEYS),
    ...
    probe = DoomWebDataset.from_local(index_path, vocab=KeyVocab(tuple(action_config.valid_keys)))
