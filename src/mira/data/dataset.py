"""DoomWebDataset: load time-aligned 4-perspective clips of Rocket League matches.
...
from .actions import KeyVocab, lines_have_mouse, tensorize_actions, tensorize_mouse
...
class MatchClip:
    ...
    mouse: "torch.Tensor | None" = None  # (P, T, 2) float32 mouse/analog deltas, or None when the
    # dataset carries no `"mouse"` field (keyboard-only data, e.g. Rocket League). Frame-aligned with
    # `actions`; consumed by the action encoder's mouse pathway (Doom's continuous turn delta is
    # written to channel 0 by the preprocessor).
...
class DoomWebDataset:
    ...
    @classmethod
    def from_local(cls, path: str | Path, **kwargs) -> "DoomWebDataset":
    ...
    def _assemble(
        ...
        # Optional analog/mouse stream. Only built when the data carries a `"mouse"` field (Doom's
        # continuous turn delta); keyboard-only data leaves this None so the loader falls back to
        # zeros -- identical to the prior behaviour. Same stride/padding contract as `actions`.
        mouse = None
        if lines_have_mouse(chunk[sel[0]]["lines"][l0:l_end]):
            per_mouse = []
            for i in sel:
